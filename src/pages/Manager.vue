<template>
	<div>
			...
			<div class='container'>
				<p>
					...
					<link-component :url="`https://github.com/ebkr/r2modmanPlus/releases/tag/${updateTagName}`"
						:target="'external'">Click here to go to the release page.
					</link-component>
				</p>
			</div>
		</div>
		...
		<div id='ror2IncorrectDir' :class="['modal', { 'is-active': (showRor2IncorrectDirectoryModal !== false) }]">
			...
			<button class="modal-close is-large" aria-label="close" @click="showRor2IncorrectDirectoryModal = false"></button>
		</div>
		<modal v-show="fixingPreloader" :open="fixingPreloader" @close-modal="closePreloaderFixModal">
			...
			<template v-slot:footer>
				<button v-if="dependencyListDisplayType === 'view'" class="button is-info" @click="closePreloaderFixModal()">
					I understand
				</button>
			</template>
		</modal>
		<modal v-if="showDependencyStrings" :open="showDependencyStrings" @close-modal="showDependencyStrings = false;">
			...
			<template v-slot:footer>
				<button class="button is-info" @click="showDependencyStrings = false;">
					Close
				</button>
			</template>
		</modal>
		<modal v-show="showLaunchParameterModal === true" :open="showLaunchParameterModal"
			@close-modal="() => { showLaunchParameterModal = false; }">
			<template v-slot:title>
...
export default class Manager extends Vue
{
	...
	isRunningRawMode: boolean = false;
	showRunRawModal: boolean = false;
	...
	showChangeAppid()
	{
		this.isRunningRawMode = this.settings.getContext().gameSpecific.runRaw;
		this.showRunRawModal = true;
	}

	updateRunRawMode()
	{
		this.settings.setRunRawMode(this.isRunningRawMode);
		this.showRunRawModal = false;
	}
