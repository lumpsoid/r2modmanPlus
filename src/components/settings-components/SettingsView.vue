<template>
    <div>
        <Hero title='Settings' :subtitle='`Advanced options for ${appName}: ` + managerVersionNumber.toString()'
            heroType='is-info' />
        <div class="margin-right">
            <div class="sticky-top sticky-top--opaque sticky-top--no-shadow sticky-top--no-padding">
                ...
                <div class="tabs">
                    <ul>
                        <li v-for="(key, index) in tabs" :key="`tab-${key}`" :class="[{ 'is-active': activeTab === key }]"
                            @click="changeTab(key)">
                            <a>{{ key }}</a>
                        </li>
                    </ul>
...
export default class SettingsView extends mixins(UtilityMixin) {
    ...
    private settingsList = [
        ...
        new SettingsRow(
            ...
            () =>
            {
                this.settings.setRunRawMode(
                    !this.settings.getContext().gameSpecific.runRaw
                );
            },
        ),
        new SettingsRow(
